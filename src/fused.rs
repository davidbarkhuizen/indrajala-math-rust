...
pub fn layer_accumulate_gradient_batch(
    ...
    // grad_w + delta_batch.T @ X in the product's own store, bit-identical to the separate add
    let new_grad_w = matmul_add(grad_w, &delta_batch.transpose(), input_activation_batch)?;
